<script lang="ts">
	...
	import { cartItemsStore, removeFromCart } from '$lib/services/cart.service';
		...
		<div class="flex-col overflow-y-auto">
			{#each $cartItemsStore as cartItem}
				<div class="flex mx-5 mb-5 gap-5">
					<img src="{cartItem.thumbnail}" width="92" height="92" alt="{cartItem.name} thumbnail" />

					<div>
						<a href="/products/{cartItem.id}" target="_self">{cartItem.name}</a>
						<div class="flex gap-3 my-2">
							<QuantityInput bind:count={cartItem.quantity} mini={true} max={cartItem.quantityItems} />
							<button
		...
		<div class="flex flex-col justify-center h-full">
			...
			<div class="px-10 py-10">
				<a href="/products" target="_self">
					<button class="w-full h-12 text-black font-bold transition-colors duration-150 bg-yellow-300 focus:shadow hover:bg-yellow-500">
						Start shopping
					</button>
				</a>
			</div>
